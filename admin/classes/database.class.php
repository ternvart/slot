...
class database
{
    ...
    {
        $stmt = self::$db->prepare("SELECT * FROM contents ORDER BY ID");
        $stmt->execute();
    ...
    {
        $stmt = self::$db->prepare("SELECT * FROM downloads ORDER BY ID");
        $stmt->execute();
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        if (!empty($rows)) {
            return $rows;
    ...
    {
        $stmt = self::$db->prepare("UPDATE proxies SET ip=:ip, port=:port, username=:proxy_user, password=:proxy_pass, usage_count=:usage_count, banned=:banned WHERE ID=:id");
        $update = $stmt->execute(array(
            "id" => $proxy["id"],
            "ip" => $proxy["ip"],
            "port" => $proxy["port"],
    ...
    {
        $stmt = self::$db->prepare("INSERT INTO proxies SET ip=:ip, port=:port, username=:proxy_user, password=:proxy_pass, usage_count=:usage_count, banned=:banned");
        $insert = $stmt->execute(array(
            "ip" => $proxy["ip"],
            "port" => $proxy["port"],
            "proxy_user" => $proxy["username"],
